	...
	Visit(Path[] cacheFiles, String delim, long numUrls) throws IOException {
		...
		if ((null != cacheFiles) && (cacheFiles.length > 0)) {
			for (Path cachePath : cacheFiles) {
				...
				if (cachePath.getName().contains(HaoGenHiveData.uagentf)) {
				...
				} else if (cachePath.getName().contains(HaoGenHiveData.countryf)) {
				...
				} else if (cachePath.getName().contains(HaoGenHiveData.searchkeyf)) {
